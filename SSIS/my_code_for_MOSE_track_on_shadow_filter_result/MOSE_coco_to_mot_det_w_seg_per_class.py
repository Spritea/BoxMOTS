...
if __name__ == "__main__":
    coco_result_dir = "../my_code_for_MOSE_shadow_combine/filter_result_MOSE/MOSE_car_seq_out_pair_warp/train_car_seq"
    outdir = coco_result_dir.replace("../my_code_for_MOSE_shadow_combine", "for_tracking_MOSE")
    # outdir = str(Path(outdir).parent)
    outdir = os.path.join(outdir, "to_mots_txt/mots_det_seg/shadow_filter_result_rate_02/car")
    os.makedirs(outdir, exist_ok=True)
    seq_list = os_sorted(os.listdir(coco_result_dir))
    seq_ped_det = []
    for seq_one in tqdm(seq_list):
        coco_result_seq_one = os.path.join(coco_result_dir, seq_one, "shadow_filter_result_rate_02.json")
