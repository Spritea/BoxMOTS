        ...
        for i in range(num_instances):
            ...
            if labels is not None:
                ...
                if boxes is not None:
                    ...
                    # added by cws.
                    # since masks[i].mask could only have 1 or 2 pixel as output,
                    # and it cannot be converted to bbox by masks[i].bbox(),
                    # but masks[i].bbox() could be wrong even when masks[i].mask has 5 pixels,
                    # so need to use try-except.
                    # if np.count_nonzero(masks[i].mask)<=5:
                    #     continue
                    try:
                        x0, y0, x1, y1 = masks[i].bbox()
                    except IndexError:
                        print('Cannot convert the mask to bounding box!')
                        continue
                ...
                # below comment by cws to not show text.
                # self.draw_text(
                #     labels[i],
                #     text_pos,
                #     color=lighter_color,
