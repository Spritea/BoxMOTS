...
def get_img_id_dict():
    # get the dict of {file_name: img_id}.
    label_file = '../../my_dataset/bdd100k/labels/seg_track_20/from_rles/val_seg_track.json'
    with open(label_file,'r') as f:
        label_content = json.load(f)
    images = label_content['images']
    img_id_dict = {x['file_name'].split('/')[1]:x['id'] for x in images}
    return img_id_dict    

def get_img_my_id_from_json_dict(file_name,img_id_dict):
    img_name = file_name.split('/')[-1]
    img_my_id = img_id_dict[img_name]
    return img_my_id
...
def instance_to_coco_json_result(instances, img_path, img_id_dict):
    ...
    img_id = get_img_my_id_from_json_dict(img_path,img_id_dict)
    result_img_one = []
...
def get_parser():
    ...
    parser.add_argument(
        "--output",
        default="./my_vis_result/bdd_mots_val/",
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            if instances is None:
                ...
                continue
            result_img_one = instance_to_coco_json_result(instances, path, img_id_dict)
        ...
        # save json to disk.
        json_out_dir = './my_json_result/bdd_mots_val/'
        os.makedirs(json_out_dir,exist_ok=True)
        json_out_path = json_out_dir+'shadow_instance_result.json'
