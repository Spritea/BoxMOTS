...
class TrackResult(Result):
    ...
    # below by cws.
    num_obj: List[Dict[str, int]]
    num_match: List[Dict[str, int]]
    num_dets: List[Dict[str, int]]
    my_smota: List[Dict[str, float]]
...
def evaluate_single_class(
    ...
    # set OVERALL to nan,
    # since when OVERALL is not nan,
    # summary["motp"] * num_dets["num_detections"] would
    # generate one more result for the OVERALL item.
    my_motp=copy.deepcopy(summary["motp"])
    my_motp['OVERALL']=np.nan
    
    sum_motp = (my_motp * num_dets["num_detections"]).sum()
        
    num_sws = mh.compute_many(
        accs,
        names=names,
        metrics=["num_switches"],
        generate_overall=True,
    )
    num_fps = mh.compute_many(
        accs,
        names=names,
        metrics=["num_false_positives"],
        generate_overall=True,
    )
    num_objs = mh.compute_many(
        accs,
        names=names,
        metrics=["num_objects"],
        generate_overall=True,
    )
    my_sum_motp=num_dets["num_detections"]["OVERALL"]-sum_motp
    smota = mm.math_util.quiet_divide(
        my_sum_motp-num_sws["num_switches"]["OVERALL"]-num_fps["num_false_positives"]["OVERALL"], num_objs["num_objects"]["OVERALL"]
    )
    flat_dict["my_smota"] = float(smota)
    # above by cws.
