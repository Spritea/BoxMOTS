...
from scalabel.label.typing import Poly2D
from scalabel.label.to_coco import get_instance_id, poly2ds_list_to_coco, set_rle_object_geometry

# scalabel2coco_seg_track by cws
# copy and modified from E:\Anaconda3\envs\bdd\Lib\site-packages\scalabel\label\to_coco.py.

def scalabel2coco_seg_track_my(
    ...
    for video_anns in tqdm(frames_list):
        ...
        # for image_anns in frames:
            ...
            file_name_list = image_anns.name.split('-')
            img_folder = file_name_list[0]+'-'+file_name_list[1]
            image = ImgType(
                video_id=video_id,
                frame_id=image_anns.frameIndex,
                file_name=img_folder+'/'+image_anns.name,
...
def main() -> None:
            ...
            if args.mask_base is not None:
                ...
                convert_func = partial(
                    dict(
                        ...
                        seg_track=scalabel2coco_seg_track_my,
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        # changed by cws to format json by adding indent=2.
