...
# below by cws.
# goal is to control num digits.
from tabulate import tabulate
def my_print(results):        
    def convert_col(metric_one):
        col_values=[x.values() for x in metric_one]
        col_flat=[x for subset in col_values for x in subset]
        return col_flat
    numbers = {'MOTA': convert_col(results.MOTA), 'MOTP': convert_col(results.MOTP), 'IDF1': convert_col(results.IDF1),
               'FP': convert_col(results.FP), 'FN': convert_col(results.FN), 'IDSw': convert_col(results.IDSw),
               'MT': convert_col(results.MT), 'PT': convert_col(results.PT), 'ML': convert_col(results.ML),
               'FM': convert_col(results.FM), 'num_obj': convert_col(results.num_obj), 'num_match': convert_col(results.num_match),
               'num_dets': convert_col(results.num_dets), 'my_smota': convert_col(results.my_smota)}
    row_name={'Category':['pedestrian','rider','car','truck','bus','train','motorcycle','bicycle',
                          'human','vehicle','bike','AVERAGE','OVERALL']}
    row_name.update(numbers)
    tab_rlt=tabulate(row_name,headers='keys',floatfmt='.4f',numalign='right')
    print(tab_rlt)
