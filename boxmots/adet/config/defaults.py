...
# added by cws.
# below for reid.
_C.MODEL.CONDINSTREID=CN()
_C.MODEL.CONDINSTREID.REID_BRANCH_OUT_CHANNELS=32
# float point need add .
_C.MODEL.CONDINSTREID.REID_LOSS_WEIGHT=1.0
# frame link means how many consecutive frames are combined,
# eg, if batch size=8, frame link=2, 
# the frame ids in one batch coud be [1,2,5,6,3,4,7,8],
# if frame link=4, then the id frames could be [5,6,7,8,1,2,3,4].
_C.DATALOADER.FRAME_LINK=8
# below for boxinst+opt flow.
_C.MODEL.BOXINST.FLOW_THRESH=0.6
_C.MODEL.BOXINST.FLOW_LOSS_FACTOR=1.0
_C.MODEL.BOXINST.USE_COLOR_SIMILARITY=True
_C.MODEL.BOXINST.USE_FLOW_SIMILARITY=False

# below for pairwarp loss.
_C.MODEL.BOXINST.PAIRWARP_LOSS_FACTOR=1.0
_C.MODEL.BOXINST.PAIRWARP_CONF_THRESH=0.5

# below for car shadow.
_C.MODEL.BOXINST.SHADOW_RATE_THRESH=0.2
_C.MODEL.BOXINST.SHADOW_LOSS_FACTOR=1.0
_C.MODEL.BOXINST.SHADOW_WARM_UP=False

# below for optical flow with gauss noise
_C.OPTICAL_FLOW_WITH_GAUSS_NOISE=CN()
_C.OPTICAL_FLOW_WITH_GAUSS_NOISE.GAUSS_NOISE_STD=1

# below for optical flow with constant noise
_C.OPTICAL_FLOW_WITH_CONSTANT_NOISE=CN()
_C.OPTICAL_FLOW_WITH_CONSTANT_NOISE.CONSTANT_NOISE_VAL=100