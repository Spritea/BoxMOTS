                ...
                if self.boxinst_enabled:
                    ...
                    if len(pair_cur_inst_mask_list)>0:
                        ...
                        if self.last_loss is not None:                           
                            ...
                            # because this situation that pair_cur_inst_mask_list is empty
                            # happens a lot in youtube_vis_2019 dataset, we should carefully
                            # set loss_pairwarp to 0.0, instead of using self.last_loss.
                            # the zero loss setting way is given by chatgpt 4.0.
                            logger.info('pair_cur_inst_mask_list is empty, set loss_pairwarp to zero to ignore loss_pairwarp for this batch.')
                            zero_loss = torch.tensor(0.0, requires_grad=True).to(device=self.last_loss.device)
                            losses.update({"loss_pairwarp": zero_loss})
