...
from .dynamic_mask_head_pair_warp import build_dynamic_mask_head
from .mask_branch import build_mask_branch

from adet.utils.comm import aligned_bilinear
# below by cws.
from .reid_one_class_infer_and_eval_branch_for_infer import build_reid_branch
from pathlib import Path
import numpy as np

__all__ = ["CondInst_ReID_One_Class_Infer_Pair_Warp"]
...
# below by cws to get opt flow motion vector.
# def get_flow_data(batched_inputs,device):
#     file_name_list=[x['file_name'] for x in batched_inputs]
#     flow_file_dir="my_opt_flow_results/KITTI_MOTS/train_in_trainval"
#     flow_data_list=[]
#     for img1_file,img2_file in zip(file_name_list[::2],file_name_list[1::2]):
#         seq_id=Path(img1_file).parent.name
#         img1_id=Path(img1_file).stem
#         img2_id=Path(img2_file).stem
#         flow_file_name='_'.join([seq_id,img1_id,img2_id])+'.npy'
#         flow_file_path=flow_file_dir+'/'+seq_id+'/ckpt_sintel_np_offset_mat/'+flow_file_name
#         with open(flow_file_path,'rb') as f:
#             flow_data_one=torch.from_numpy(np.load(f)).to(device)
#             flow_data_list.append(flow_data_one)
#     return flow_data_list

def pad_flow_data(original_images,flow_data):
    # This is to make flow data have same size with padded original_images/original_image_masks
    # in the same batch that have been combined to one tensor by 
    # original_images = ImageList.from_tensors(original_images, self.backbone.size_divisibility)-[a].
    # no use flow_data_pad = ImageList.from_tensors([flow_data[0]], self.backbone.size_divisibility)-[b].
    # For [a], it would pad all images in the origial_images list, bsed on the max img size in
    # the origial_images list, and the size_divisibility.
    # For [b], it pads the [flow_data[0]] only based on the flow_data itself.
    # [flow_data[0]] have the same height and width with its corresponding img in the origial_images list.
    # So if we want to make [flow_data[0]] to be padded to the same size of padded original_images,
    # [flow_data[0]] must have the max img size along original_images list,
    # this means each img in original_images list have the max img size along original_images list,
    # this means that each img after resizeshortestedge transform has basically same size. 
    # This is true for KITTI data.
    # But for BDD data, the images after resizeshortestedge transform have very different img sizes,
    # so [b] would get different padded sizes for [flow_data[0]] on different imgs,
    # not the same size with the padded original_images.
    # Hence we need to explicitly pad the [flow_data[0]] to the same size with the padded original_images.
    target_h,target_w = original_images.tensor.shape[-2],original_images.tensor.shape[-1]
    flow_h,flow_w = flow_data.shape[-2],flow_data.shape[-1]
    padding_size = [0, target_w - flow_w, 0, target_h - flow_h]
    flow_data_pad = F.pad(flow_data[0], padding_size, value=0.0)
    return flow_data_pad

@META_ARCH_REGISTRY.register()
class CondInst_ReID_One_Class_Infer_YTVIS_Pair_Warp_ReID_Eval_In_Train_For_Infer(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.boxinst_enabled:
                ...
                for i in range(len(original_image_masks)):
                    ...
                    # TODO: may not need this for kitti mots dataset.
                    # this is needed for COCO dataset.
                    pixels_removed = int(
                ...
                # )
                # add self.training condition is for eval without flow data.
                if self.training:
                    # below pad and downsample flow data.
                    for inst_one in gt_instances:
                        flow_data = inst_one.flow_data
                        # flow_data_pad = ImageList.from_tensors(
                        #     [flow_data[0]], self.backbone.size_divisibility)
                        flow_data_pad = pad_flow_data(original_images,flow_data)
                        # flow_data_pad = flow_data_pad.tensor[0]
                        
                        start = int(self.mask_out_stride // 2)
                        # TODO:maybe use F.avg_pool2d to do downsample, like image downsample.
                        flow_data_downsample = flow_data_pad[:,
                                                            start::self.mask_out_stride, start::self.mask_out_stride]
                        #when do downsample, the flow data (tracking offset) also needs downsample.
                        flow_data_downsample = flow_data_downsample/self.mask_out_stride
                        inst_one.flow_data = torch.repeat_interleave(
                            torch.unsqueeze(flow_data_downsample, dim=0), len(inst_one), dim=0)

                    self.add_bitmasks_opt_flow_from_boxes(
                        gt_instances, original_images.tensor, original_image_masks.tensor,
                        original_images.tensor.size(
                            -2), original_images.tensor.size(-1)
                    )
            else:
                self.add_bitmasks(
        ...
        if self.training:
            ...
            for im_id, (input_per_image, image_size) in enumerate(zip(batched_inputs, images_norm.image_sizes)):
                ...
                instances_per_im_for_backbone = pred_instances_w_masks[
                ...
                reid_out = self.reid_branch(
                    features, gt_instances=gt_instances, pred_instances=instances_per_im_for_backbone)
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances):
        ...
        if self.max_proposals != -1:
            if self.max_proposals < len(pred_instances):
                inds = torch.randperm(
                    len(pred_instances), device=mask_feats.device).long()
            ...
            for im_id in range(num_images):
                ...
                for gt_ind in unique_gt_inds:
                    ...
                    if len(instances_per_gt) > num_instances_per_gt:
                        scores = instances_per_gt.logits_pred.sigmoid().max(dim=1)[
                            0]
                        ctrness_pred = instances_per_gt.ctrness_pred.sigmoid()
                        inds = (
                            scores * ctrness_pred).topk(k=num_instances_per_gt, dim=0)[1]
    ...
    def add_bitmasks(self, instances, im_h, im_w):
        for per_im_gt_inst in instances:
            ...
            if isinstance(per_im_gt_inst.get("gt_masks"), PolygonMasks):
                ...
                per_im_gt_inst.gt_bitmasks = torch.stack(
                    per_im_bitmasks, dim=0)
    ...
    def add_bitmasks_opt_flow_from_boxes(self, instances, images, image_masks, im_h, im_w):
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            # below for get opt flow vec similarity.
            flow_data_per_im = per_im_gt_inst.flow_data[0].unsqueeze(dim=0)
            flow_similarity = get_images_color_similarity(
                flow_data_per_im, image_masks[im_i],
                self.pairwise_size, self.pairwise_dilation
            )
            per_im_gt_inst.flow_similarity = torch.cat([
                flow_similarity for _ in range(len(per_im_gt_inst))
            ], dim=0)
            ...
            for per_box in per_im_boxes:
                bitmask_full = torch.zeros(
                    (im_h, im_w), device=self.device).float()
