...
from .dynamic_mask_head_opt_flow import build_dynamic_mask_head
...
import numpy as np
from adet.layers import DFConv2d

__all__ = ["CondInst_ReID_One_Class_Infer_Opt_Flow_Deform_Conv"]
...
class CondInst_ReID_One_Class_Infer_Opt_Flow_Deform_Conv(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        # add deform conv for each level output of fpn.
        feature_channels = {k: v.channels for k, v in self.backbone.output_shape().items()}
        self.deform_conv_list=nn.ModuleList(DFConv2d(feature_channels[x], feature_channels[x],kernel_size=3, stride=1,padding=1, bias=True) 
                                            for x in feature_channels.keys())
    ...
    def forward(self, batched_inputs):
        ...
        # below make fpn feat pass deform conv.
        feat_keys=list(features.keys())
        for k,v in features.items():
            features[k]=self.deform_conv_list[feat_keys.index(k)](v)
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.boxinst_enabled:
                ...
                for inst_one in gt_instances:
                    ...
                    flow_data_pad=ImageList.from_tensors([flow_data[0]],self.backbone.size_divisibility)
                    flow_data_pad=flow_data_pad.tensor[0]
                    start = int(self.mask_out_stride // 2)
                    # TODO:maybe use F.avg_pool2d to do downsample, like image downsample.
                    flow_data_downsample = flow_data_pad[:, start::self.mask_out_stride, start::self.mask_out_stride]
                    inst_one.flow_data=torch.repeat_interleave(torch.unsqueeze(flow_data_downsample,dim=0),len(inst_one),dim=0)
