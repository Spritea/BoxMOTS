...
from .dynamic_mask_head_car_shadow import build_dynamic_mask_head
...
__all__ = ["CondInst_ReID_One_Class_Eval"]
...
class CondInst_ReID_One_Class_Infer_Car_Shadow(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.boxinst_enabled:
                ...
                # below pad and downsample shadow data.
                for inst_one in gt_instances:
                    shadow_data=inst_one.shadow_data
                    shadow_data_pad=ImageList.from_tensors([shadow_data[0]],self.backbone.size_divisibility)
                    shadow_data_pad=shadow_data_pad.tensor[0]
                    start = int(self.mask_out_stride // 2)
                    # TODO:maybe use F.avg_pool2d to do downsample, like image downsample.
                    shadow_data_downsample = shadow_data_pad[:, start::self.mask_out_stride, start::self.mask_out_stride]
                    inst_one.shadow_data=torch.repeat_interleave(torch.unsqueeze(shadow_data_downsample,dim=0),len(inst_one),dim=0)
