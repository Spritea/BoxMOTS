...
from .dynamic_mask_head_mask_pool import build_dynamic_mask_head
from .mask_branch import build_mask_branch

from adet.utils.comm import aligned_bilinear
# below by cws.
from .reid_one_class_mask_pool_infer_branch import build_reid_branch


__all__ = ["CondInst_ReID_One_Class_Mask_Pool_Infer"]
...
class CondInst_ReID_One_Class_Mask_Pool_Infer(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            mask_losses,pred_instances = self._forward_mask_heads_train_out_pred(proposals, mask_feats, gt_instances)
            reid_losses = self.reid_branch(features, gt_instances=gt_instances,
                                           pred_masks=self.mask_head.mask_score_reid,
            ...
            for im_id, (input_per_image, image_size) in enumerate(zip(batched_inputs, images_norm.image_sizes)):
                ...
                reid_out=self.reid_branch(features,pred_masks=self.mask_head.mask_score_reid,
                                          pred_instances=instances_per_im_for_backbone)
                processed_results.append(reid_out)
                
            return processed_results
        
    def _forward_mask_heads_train_out_pred(self, proposals, mask_feats, gt_instances):
        ...
        return loss_mask,pred_instances
