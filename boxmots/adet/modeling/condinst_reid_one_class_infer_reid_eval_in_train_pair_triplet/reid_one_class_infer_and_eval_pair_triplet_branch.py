...
from my_code.my_loss import PairTripletLoss
INF = 100000000
...
class ReIDBranchOneClass(nn.Module):
    ...
    def forward(self, features, gt_instances=None,pred_instances=None):
        ...
        def reid_infer(pred_instances):
            ...
            #     print('kk')
            # when modify outer variable, must either pass it to the inner function
            # and change it, or use non_local/global to clarify the outer variable,
            # but read the outer variable only is ok without any extra work.
        ...
        if self.training:
            ...
            # add inst_num_by_img to know which img one inst belongs to,
            # this is for pair triplet loss.
            inst_num_by_img=[len(x) for x in gt_instances]
            loss=self.trip_loss(x_fc_2,track_id_tensor,inst_num_by_img)
        ...
        # the input is boxes produced by detection branch.
            ...
            reid_infer_pred=reid_infer(pred_instances=pred_instances)
            reid_eval_gt=reid_eval()
            
            return {'reid_infer_pred':reid_infer_pred,'reid_eval_gt':reid_eval_gt}
