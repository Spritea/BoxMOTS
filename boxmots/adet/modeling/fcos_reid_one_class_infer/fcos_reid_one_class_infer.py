...
__all__ = ["FCOS_ReID_One_Class_Eval"]
...
class FCOS_ReID_One_Class_Infer(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        # )
        proposals, proposal_losses = self.proposal_generator(
            images_norm, features, gt_instances
        )
        ...
        if self.training:
            ...
            for im_id, (input_per_image, image_size) in enumerate(zip(batched_inputs, images_norm.image_sizes)):
                ...
                instances_per_im_for_backbone = pred_instances
                instances_per_im = self.postprocess(
