...
for img_id_one in tqdm(img_id_list):
    ...
    if len(shadow_mask_img_one)>0:
        for ctr,boxinst_inst_one in enumerate(boxinst_img_one):
            # only for ped class.
            if boxinst_inst_one['category_id']==0:
...
json_outpath = '../../my_shadow_det/filter_result/pairwarp_result_pretrain_weights_v2/shadow_filter_result_rate_0'+str(int(shadow_rate_th*10))+'_use_ped.json'
