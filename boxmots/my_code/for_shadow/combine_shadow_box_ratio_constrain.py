...
boxinst_filepath = "../../my_shadow_det/original_boxinst_result/pairwarp_result_pretrain_weights_v2/inference/iter_0003999/coco_instances_results.json"
...
for img_id_one in tqdm(img_id_list):
    ...
    if len(shadow_mask_img_one)>0:
        for ctr,boxinst_inst_one in enumerate(boxinst_img_one):
            # only for car class.
            vertical = boxinst_inst_one['bbox'][3]/boxinst_inst_one['bbox'][2]>1.5
            if boxinst_inst_one['category_id']==1 and not vertical:
...
json_outpath = '../../my_shadow_det/filter_result/pairwarp_result_pretrain_weights_v2/shadow_filter_result_rate_0'+str(int(shadow_rate_th*10))+'_box_ratio.json'
