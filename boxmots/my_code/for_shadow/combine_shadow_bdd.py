...
boxinst_filepath = "../../my_shadow_det/original_boxinst_result/bdd_data/boxinst/COCO_pretrain_strong/bs4/inference_final/coco_instances_results.json"
...
for img_id_one in tqdm(img_id_list):
    ...
    if len(shadow_mask_img_one)>0:
        for ctr,boxinst_inst_one in enumerate(boxinst_img_one):
            # only for car class.
            if boxinst_inst_one['category_id']==3:
...
json_outpath = '../../my_shadow_det/filter_result/bdd_data/boxinst/COCO_pretrain_strong/bs4/inference_final/shadow_filter_result_rate_0'+str(int(shadow_rate_th*10))+'.json'
