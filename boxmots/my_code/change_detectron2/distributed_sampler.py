...
class TrainingSampler(Sampler):
    ...
    def __iter__(self):
        # changed by cws.
        # to make each gpu have consecutive frames, no shuffle used.
        if self._shuffle:
            start = self._rank
    ...
    def _infinite_indices(self):
        ...
        while True:
            if self._shuffle:
                ...
                # step_size=self._size//self._world_size
                # yield from torch.arange(self._size).tolist()[self._rank*step_size:self._rank*step_size+step_size]

class TrainingSamplerSeqShuffle(Sampler):
    """
    by cws.
    This is to shuffle for video sequences.
    """

    def __init__(self, size: int, total_batch_size: int, frame_link:int,shuffle: bool = True, seed: Optional[int] = None):
        """
        Args:
            size (int): the total number of data of the underlying dataset to sample from
            shuffle (bool): whether to shuffle the indices or not
    ...
    def _infinite_indices(self):
        ...
        while True:
            if self._shuffle:
                ...
                # changed by cws.
                id_list=list(range(self._size))
                random.seed(self._seed)
                start_id=random.sample(range(self._fl),1)[0]
                
                out_all=[]
                for k in range(len(id_list)//self._fl):
                    end_id=start_id+self._fl*(k+1)
                    if end_id<len(id_list):
                        out_one=id_list[start_id+self._fl*k:end_id]
                ...
                # yield from torch.arange(self._size).tolist()
                # below for multi gpu.
                size_per_gpu=len(list_out)//self._world_size
                list_per_gpu=list_out[self._rank*size_per_gpu:(self._rank+1)*size_per_gpu]
                yield from list_per_gpu
