...
file_dir="../my_data/reid_one_class_infer_for_train/COCO_pretrain_strong/search_for_best_iter/long_epoch_one_gpu_seq_shuffle_fl_2_lr_0_0001_bs_4_eval_500_steps_4k/BoxInst_MS_R_50_1x_kitti_mots/to_mots_txt/mots_seg_track/DeepSORT/val_in_trainval/min_det_conf_06/shadow_filter_result_rate_02/"
for seq_one in tqdm(seqmap):
    ...
    out_dir=file_dir+'both_car_pedestrian_no_overlap_keep_ped/'
    # out_dir=file_dir+'tt/'
    ...
    for frame_id in range(0,max_frame+1):
        ...
        for item in frame_one_content:
            ...
            # make car -1, and ped -2, so we can make ped first.
            category_list.append(int(item.split(' ')[2])*(-1))
        objects_sort_no_category=[x for _,x in sorted(zip(area_list,frame_one_content),key=lambda pair:pair[0])]
        objects_sort=[x for _,x in sorted(zip(category_list,objects_sort_no_category),key=lambda pair:pair[0])]
